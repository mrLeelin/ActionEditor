...
namespace NBCActionEditor.Editor.GUIS
{
    /// <summary>
    /// EditorGUIEx
    /// </summary>
    public static class EditorGUIEx
    {
        ...
        public static bool MinMaxSlider(Rect position, ref float minValue, ref float maxValue, float minLimit,
            float maxLimit)
        {
            return DoMinMaxSlider(EditorGUI.IndentedRect(position),
                GUIUtility.GetControlID(_MinMaxSliderHash, FocusType.Passive), ref minValue, ref maxValue, minLimit,
                maxLimit);
        }

        private static bool DoMinMaxSlider(Rect position, int id, ref float minValue, ref float maxValue,
        ...
        private static bool _DoMinMaxSlider(Rect position, int id, ref float value, ref float size, float minLimit,
            float maxLimit)
        {
            ...
            Rect thumbMin = new Rect(thumbCenter.x - minMaxThumbWidth, thumbCenter.y, minMaxThumbWidth,
                thumbCenter.height);
            Rect thumbMax = new Rect(thumbCenter.x + thumbCenter.width, thumbCenter.y, minMaxThumbWidth,
                thumbCenter.height);
            ...
            switch (evt.GetTypeForControl(id))
            {
                case EventType.MouseDown:
                    ...
                    if (thumbRt.Contains(evt.mousePosition))
                    {
                        ...
                        _stateEx.whereWeDrag = thumbMin.Contains(evt.mousePosition) ? 1 :
                            thumbMax.Contains(evt.mousePosition) ? 2 : 0;
                ...
                case EventType.Repaint:
                    ...
                    EditorGUIUtility.AddCursorRect(thumbMin, MouseCursor.ResizeHorizontal,
                        _stateEx != null && _stateEx.whereWeDrag == 1 ? id : -1);
                    EditorGUIUtility.AddCursorRect(thumbMax, MouseCursor.ResizeHorizontal,
                        _stateEx != null && _stateEx.whereWeDrag == 2 ? id : -1);
